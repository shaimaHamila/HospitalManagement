    ...
    public class MedecinDAO
    {
        ...
        public List<Medecin> FindAll()
        {
            ...
            try
            {
                ...
                MySqlCommand cmd = new MySqlCommand("select * from medecin", cnx);
        ...
        public Medecin Find(String cin)
        {
            ...
            {
                cnx = ConnexionHopital.GetInstance();
                MySqlCommand cmd = new MySqlCommand("select * from medecin where cin=@cin", cnx);
                cmd.Parameters.Add("@cin", cin);
                MySqlDataReader reader = cmd.ExecuteReader();
                while (reader.Read())
                {
                    m = new Medecin(reader.GetString("cin"), reader.GetString(1), reader.GetString(2), reader.GetDateTime(3), reader.GetString(4));
        ...
        public void Remove(Medecin m)
        {
            ...
            {
                cnx = ConnexionHopital.GetInstance();
                MySqlCommand cmd = new MySqlCommand("delete from medecin where cin=@cin", cnx);
        ...
        public void Add(Medecin m)
        {
            ...
            {
                cnx = ConnexionHopital.GetInstance();
                MySqlCommand cmd = new MySqlCommand("insert into medecin values(@cin, @prenom, @nom, @nais, @adr)", cnx);
                cmd.Parameters.Add("@cin", m.Cin);//
        ...
        public List<Medecin> FindToAdresse(String adresse)
        {
            ...
            {
                cnx = ConnexionHopital.GetInstance();
                MySqlCommand cmd = new MySqlCommand("select * from medecin where adresse=@adre", cnx);
        ...
        public List<Medecin> FindToNom(String nom)
        {
            ...
            {
                cnx = ConnexionHopital.GetInstance();
                MySqlCommand cmd = new MySqlCommand("select * from medecin where nom=@nom", cnx);
        ...
        public void Update(Medecin m)
        {
            ...
            {
                cnx = ConnexionHopital.GetInstance();
                MySqlCommand cmd = new MySqlCommand("update medecin set  nom=@nom, prenom=@prenom, datenaissance=#nais, adresse=@adr where cin=@cin", cnx);
                cmd.Parameters.Add("@cin", m.Cin);//
                cmd.Parameters.Add("@nom", m.Nom);//
                cmd.Parameters.Add("@prenom", m.Prenom);//
                cmd.Parameters.Add("@nais", m.DateNaissance);//
